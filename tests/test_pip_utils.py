...
class TestFindRequirements(SimpleTestCase):
    ...
    def test_find_outdated_libraries(self, mock_get):
        ...
        self.assertEqual(results['celery']['comes_from']['file_indicator'], '-r')
        self.assertTrue(filename in results['celery']['comes_from']['filename'])
        self.assertTrue(filename in results['celery']['comes_from']['value'])
        self.assertEqual(results['celery']['comes_from']['line_no'], 2)
        self.assertEqual(results['celery']['latest_version'], '4.1.0')
        self.assertEqual(results['celery']['name'], 'celery')
        self.assertEqual(results['celery']['operator'], '==')
        self.assertEqual(results['celery']['version'], '4.0.1')
        self.assertEqual(results['django']['comes_from']['file_indicator'], '-r')
        self.assertTrue(filename in results['django']['comes_from']['filename'])
        self.assertEqual(results['django']['comes_from']['line_no'], 1)
        self.assertEqual(results['django']['latest_version'], '2.0')
        self.assertTrue(filename in results['django']['comes_from']['value'])
        self.assertEqual(results['django']['name'], 'django')
        self.assertEqual(results['django']['operator'], '==')
        self.assertEqual(results['django']['version'], '1.11.3')
